...
namespace MainForm.Algorithms
{
    public static class CryptoAlgorithms
    {
        ...
        // Bu algoritma iki farklı anahtar kelime destekler: "ANAHTAR1,ANAHTAR2" formatında.
        // Eğer tek anahtar girilirse, ikincisi otomatik olarak ilkinin tersinden üretilir.

        public static string FourSquareEncrypt(string text, string key)
        {
            text = CleanInput(text);
            if (text.Length % 2 != 0) text += "X"; // Harf sayısı tekse sonuna 'X' ekle

            // Anahtarları parse etme (virgül kontrolü)
            string key1, key2;
            if (key.Contains(","))
            {
                var keys = key.Split(',');
                key1 = CleanInput(keys[0]);
                key2 = CleanInput(keys[1]);
            }
            else
            {
                key1 = CleanInput(key);
                if (string.IsNullOrWhiteSpace(key1)) key1 = "ANAHTAR";
                key2 = new string(key1.Reverse().ToArray()); // Tek anahtar varsa tersini kullan
            }

            string normalSquare = AlphabetTR + "X"; // 30 karakterlik standart tablo
            ...
            for (int i = 0; i < text.Length; i += 2)
            {
                ...
                // Karakter tabloda yoksa (boşluk vb.) son karakteri baz al
                ...
                // Kesişim noktalarını şifreli karelerden al
        ...
        {
            // Anahtar kelime + Alfabe + Dolgu karakteri kombinasyonuyla benzersiz tablo oluşturur
        ...
        public static string FourSquareDecrypt(string text, string key)
        {
            ...
            // Şifre çözerken de aynı anahtar ayrıştırma mantığını kullanıyoruz
            string key1, key2;
            if (key.Contains(","))
            {
                var keys = key.Split(',');
            ...
            else
            {
                key1 = CleanInput(key);
                if (string.IsNullOrWhiteSpace(key1)) key1 = "ANAHTAR";
                key2 = new string(key1.Reverse().ToArray());
            }
            ...
            for (int i = 0; i < text.Length; i += 2)
            {
                ...
                // Şifreli harfleri standart tabloya geri dönüştür
